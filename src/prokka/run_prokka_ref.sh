...
# Define variables
tool_name=prokka
outputfolder=${p}/data/ref/$tool_name
...
# Define variables
samples_path=${p}/data/ref
samples=$(ls $samples_path | grep '.fna')
count=$((1))
total=$(wc -w <<<$samples)
...
for sample in $samples
  do
    ...
    # Define tool inputs
    input_fasta=${samples_path}/$sample

    # Run tool
    prokka --outdir $sample_path $input_fasta --force
